...
use windows::{
    ...
    Win32::System::EventLog::{
        ...
        EVT_HANDLE, EvtSubscribeToFutureEvents,
    },
};
...
fn subscribe_sysmon(
    ...
    loop {
        ...
        let result = unsafe {
            EvtNext(subscription, &mut event_raw, 500, 0, &mut returned)
        };
...
fn render_to_xml(event: EVT_HANDLE) -> Option<String> {
    ...
    let _ = unsafe {
        EvtRender(EVT_HANDLE::default(), event, EvtRenderEventXml.0,
                  0, None, &mut buf_used, &mut prop_count)
    };
    ...
    let ok = unsafe {
        EvtRender(EVT_HANDLE::default(), event, EvtRenderEventXml.0,
                  buf_used, Some(buf.as_mut_ptr() as *mut _),
                  &mut buf_used, &mut prop_count)
    };
...
fn promote_fields(payload: &mut Value, data: &Map<String, Value>, event_id: u32) {
    ...
    for field in &[
        "Image", "CommandLine", "User", "ProcessGuid", "ProcessId",
        "ParentImage", "ParentCommandLine", "ParentProcessGuid",
        "TargetFilename", "TargetObject", "Details", "DestinationIp",
        "DestinationPort", "DestinationHostname", "Protocol",
        "SourceIp", "SourcePort", "ImageLoaded", "Signature",
        "SignatureStatus", "Signed", "Hashes",
        "SourceProcessGuid", "TargetProcessGuid", "StartAddress",
        "StartModule", "GrantedAccess", "CallTrace",
    ...
    match event_id {
        1 => {
            ...
            for f in &["CurrentDirectory", "IntegrityLevel", "LogonId", "LogonGuid"] {
                if let Some(v) = data.get(*f) { payload[*f] = v.clone(); }
            }
        }
        3 => {
            // NetworkConnect: promote Initiated (true = outbound)
            if let Some(v) = data.get("Initiated") { payload["Initiated"] = v.clone(); }
        }
        7 => {
        ...
        12 | 13 | 14 => {
            ...
            if let Some(v) = data.get("EventType") { payload["EventType"] = v.clone(); }
        }
        _ => {}
    }
}
...
fn extract_hash(hashes: &str, algo: &str) -> Option<String> {
    ...
    let end     = hashes[start..].find(',').map(|p| start + p).unwrap_or(hashes.len());
        ...
        if let Some(name_end) = xml[name_start..].find(q_char) {
            ...
            if let Some(gt) = xml[abs..].find('>') {
                ...
                if let Some(close) = xml[val_start..].find("</Data>") {
                    if !name.is_empty() {
                        map.insert(name, Value::String(xml[val_start..val_start + close].to_string()));
                    }
                }
            }
        }
