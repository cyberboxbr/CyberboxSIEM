...
impl Config {
    fn from_env() -> Result<Self> {
        ...
        let wel_channels: Vec<String> =
            env_str("COLLECTOR_WEL_CHANNELS", "Security,System,Application")
                .split(',')
        ...
        let netflow_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_NETFLOW_BIND")?)
            }
        };
        ...
        let json_udp_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_JSON_UDP_BIND")?)
            }
        };
        let json_tcp_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_JSON_TCP_BIND")?)
            }
        };
        let gelf_udp_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_GELF_UDP_BIND")?)
            }
        };
        let gelf_tcp_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_GELF_TCP_BIND")?)
            }
        };
        let otlp_http_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_OTLP_HTTP_BIND")?)
            }
        };
        let tls_reload_bind: Option<SocketAddr> = {
            ...
            if s.is_empty() {
                ...
                Some(s.parse().context("invalid COLLECTOR_TLS_RELOAD_BIND")?)
            }
        };
        ...
        Ok(Self {
            ...
            queue_max_mb: env_str("COLLECTOR_QUEUE_MAX_MB", "256")
                .parse()
                .context("invalid COLLECTOR_QUEUE_MAX_MB")?,
            ...
            dlq_max_mb: env_str("COLLECTOR_DLQ_MAX_MB", "64")
                .parse()
                .context("invalid COLLECTOR_DLQ_MAX_MB")?,
        })
    }
    ...
    fn multiline_config(&self) -> Result<MultilineConfig> {
        let pattern = match &self.ml_pattern {
            Some(p) => {
                Some(Regex::new(p).with_context(|| format!("invalid COLLECTOR_ML_PATTERN: {p}"))?)
            }
            None => None,
        };
        Ok(MultilineConfig {
...
async fn serve_tls_reload(
    ...
    let listener = match tokio::net::TcpListener::bind(bind).await {
        Ok(l) => l,
        Err(e) => {
            error!(%e, %bind, "TLS reload server: bind failed");
            return;
        }
    };
...
async fn main() -> Result<()> {
    ...
    let rate_limiter = Arc::new(SourceRateLimiter::new(
        cfg.source_rate_eps,
        cfg.source_rate_burst,
    ));
    if cfg.source_rate_eps > 0 {
        info!(
            eps = cfg.source_rate_eps,
            burst = cfg.source_rate_burst,
            "per-source-IP rate limiting enabled"
        );
    ...
    if let Some(ref swap) = tls_swap {
        ...
        tokio::spawn(async move {
            ...
            let mut sig = match signal(SignalKind::user_defined1()) {
                Ok(s) => s,
                Err(e) => {
                    error!(%e, "SIGUSR1 handler setup failed");
                    return;
                }
            };
            loop {
    ...
    if let (Some(reload_bind), Some(ref swap)) = (cfg.tls_reload_bind, &tls_swap) {
        ...
        warn!(
            "COLLECTOR_TLS_RELOAD_BIND is set but TLS is not configured — reload endpoint disabled"
        );
    }
