//! Windows Event Log collection using the EvtXxx API (Vista+, RFC — none).
//!
//! Each channel (Security, System, Application, …) gets its own OS thread
//! (`spawn_blocking`) that subscribes to future events and renders them as XML.
//! The XML is then parsed into a structured `raw_payload` and forwarded to the
//! shared ingest mpsc channel.
//!
//! This module is only compiled on Windows (`#[cfg(windows)]` in sources/mod.rs).
...
/// Spawn one blocking thread per channel, each publishing events to `tx`.
pub async fn run(channels: Vec<String>, tenant_id: Arc<String>, tx: mpsc::Sender<Value>) {
...
fn subscribe_channel(channel: &str, tenant_id: Arc<String>, tx: mpsc::Sender<Value>) {
    ...
    info!(channel, "Windows Event Log subscription active");

    // EvtNext expects &mut [isize] — EVT_HANDLE is a newtype over isize
    ...
    loop {
        ...
        if returned > 0 {
            ...
            if code != ERROR_NO_MORE_ITEMS.0 {
                // Genuine error — back off and retry
                warn!(channel, error = %e, "EvtNext error");
                std::thread::sleep(std::time::Duration::from_secs(1));
            }
            // ERROR_NO_MORE_ITEMS (259) is normal when no new events
        }
    }
}
...
/// Extract `<Data Name='key'>value</Data>` pairs into a Map.
        ...
        if let Some(name_end) = xml[name_start..].find(q_char) {
            ...
            if let Some(gt) = xml[abs..].find('>') {
                ...
                if let Some(close) = xml[val_start..].find("</Data>") {
                    let val = xml[val_start..val_start + close].to_string();
                    if !name.is_empty() {
