...
pub async fn load_secrets(client: &reqwest::Client) -> Result<()> {
    let addr = match std::env::var("COLLECTOR_VAULT_ADDR").ok().filter(|s| !s.is_empty()) {
        Some(a) => a,
        None    => return Ok(()), // Vault not configured
    };

    let paths_raw = std::env::var("COLLECTOR_VAULT_PATHS").unwrap_or_default();
    let paths: Vec<&str> = paths_raw.split(',').map(str::trim).filter(|s| !s.is_empty()).collect();
    if paths.is_empty() {
    ...
    let namespace = std::env::var("COLLECTOR_VAULT_NAMESPACE").ok().filter(|s| !s.is_empty());

    // Authenticate
    let token = authenticate(client, &addr, namespace.as_deref()).await
...
async fn authenticate(
    ...
    // Static token
    if let Some(tok) = std::env::var("COLLECTOR_VAULT_TOKEN").ok().filter(|s| !s.is_empty()) {
        return Ok(tok);
    }
    ...
    if let Some(ns) = namespace { req = req.header("X-Vault-Namespace", ns); }
