...
use cyberbox_collector::{metrics::CollectorMetrics, ratelimit::SourceRateLimiter};
...
async fn gelf_tcp_roundtrip() {
    ...
    let mut stream = TcpStream::connect(bound_addr).await.unwrap();
    let gelf =
        b"{\"version\":\"1.1\",\"host\":\"db-01\",\"short_message\":\"slow query\",\"level\":4}\0";
