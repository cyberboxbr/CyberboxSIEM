...
fn estr(k: &str, d: &str) -> String {
    std::env::var(k).unwrap_or_else(|_| d.to_string())
}
fn eu64(k: &str, d: u64) -> u64 {
    std::env::var(k)
        .ok()
...
impl Res {
    ...
    fn enqueued(&self) -> u64 {
        self.snap
            .as_ref()
            .map(|s| s.udp_recv + s.tcp_recv)
            .unwrap_or(0)
    }
    /// Total events parsed = enqueued + channel_drops (both prove the datagram was parsed)
    fn processed(&self) -> u64 {
        self.enqueued() + self.ch_drops()
    }
    fn recv_rate(&self) -> f64 {
        self.processed() as f64 / self.duration.as_secs_f64()
    }
    fn fwd_rate(&self) -> f64 {
        self.snap
            .as_ref()
            .map(|s| s.forwarded as f64 / self.duration.as_secs_f64())
            .unwrap_or(0.0)
    }
    fn recv_pct(&self) -> f64 {
        if self.sent > 0 {
            self.processed() as f64 * 100.0 / self.sent as f64
        } else {
            0.0
        }
    }
    fn ch_drops(&self) -> u64 {
        self.snap.as_ref().map(|s| s.ch_drops).unwrap_or(0)
    }
    fn rl_drops(&self) -> u64 {
        self.snap.as_ref().map(|s| s.rl_drops).unwrap_or(0)
    }
    fn parse_err(&self) -> u64 {
        self.snap.as_ref().map(|s| s.parse_errs).unwrap_or(0)
    }
    ...
    fn bw_mbs(&self) -> f64 {
        self.processed() as f64 * self.msg_bytes as f64 / self.duration.as_secs_f64() / 1_000_000.0
    }
}
...
async fn tcp_syslog_sender(
    ...
    let mut stream = match TcpStream::connect(target).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("TCP syslog connect failed: {e}");
            return;
        }
    };
    while Instant::now() < deadline {
...
async fn gelf_tcp_sender(
    ...
    let mut stream = match TcpStream::connect(target).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("GELF TCP connect failed: {e}");
            return;
        }
    };
    while Instant::now() < deadline {
...
async fn json_tcp_sender(
    ...
    let mut stream = match TcpStream::connect(target).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!("JSON TCP connect failed: {e}");
            return;
        }
    };
...
async fn run(
    ...
    print!(
        "  ▶ {label:<45} (c={concurrency}, ~{msg_bytes}B, {}s)… ",
        duration.as_secs()
    );
...
async fn run_combined(
    ...
    for id in 0..tasks_per_proto {
        let ctr = Arc::clone(&counter);
        tasks.push(tokio::spawn(udp_sender(
            a.udp,
            syslog_msg(id, msg_bytes),
            ctr.clone(),
            deadline,
        )));
        tasks.push(tokio::spawn(udp_sender(
            a.gelf_udp,
            gelf_msg(id, msg_bytes),
            ctr.clone(),
            deadline,
        )));
        tasks.push(tokio::spawn(tcp_syslog_sender(
            a.tcp,
            tcp_syslog_msg(id, msg_bytes),
            ctr.clone(),
            deadline,
        )));
        tasks.push(tokio::spawn(gelf_tcp_sender(
            a.gelf_tcp,
            gelf_tcp_msg(id, msg_bytes),
            ctr.clone(),
            deadline,
        )));
        tasks.push(tokio::spawn(json_tcp_sender(
            a.json_tcp,
            json_tcp_msg(id, msg_bytes),
            ctr.clone(),
            deadline,
        )));
    }
...
async fn main() -> Result<()> {
    ...
    match scrape(&client, &a.metrics).await {
        ...
        None => {
            println!("  ✗ WARNING: metrics endpoint unreachable — per-metric stats will be 0\n")
        }
    }

    // ── Test matrix ───────────────────────────────────────────────────────────
    println!(
        "  Running test matrix ({} individual tests + 1 combined)…",
        10
    );
    println!();
    ...
    macro_rules! bench {
        ($label:expr, $proto:expr, $target:expr, $c:expr, $sz:expr) => {
            results.push(
                run(
                    $label, $proto, $target, $c, $sz, a.duration, a.settle, &client, &a.metrics,
                )
                .await,
            );
        };
    ...
    if let Some(r) = udp_best {
        println!(
            "  UDP Syslog peak parsed rate    : {:>9.0} msg/s  (c={}, {}B)",
            r.recv_rate(),
            r.concurrency,
            r.msg_bytes
        );
    ...
    if let Some(r) = tcp_best {
        println!(
            "  TCP Syslog peak parsed rate    : {:>9.0} msg/s  (c={}, {}B)",
            r.recv_rate(),
            r.concurrency,
            r.msg_bytes
        );
    ...
    if let Some(r) = combined {
        println!(
            "  Combined (5 protos) send rate  : {:>9.0} msg/s  ({}B)",
            r.send_rate(),
            r.msg_bytes
        );
        println!(
            "  Combined recv rate             : {:>9.0} msg/s",
            r.recv_rate()
        );
        println!(
            "  Combined forward rate          : {:>9.0} msg/s",
            r.fwd_rate()
        );
    }
    ...
    let market = [
        Market {
            name: "Vector 0.36",
            rate: 1_500_000.0,
            notes: "Rust, vectorised, batched",
        },
        Market {
            name: "Fluent Bit 3.x",
            rate: 500_000.0,
            notes: "C, lightweight, async",
        },
        Market {
            name: "rsyslog 8.x (imudp)",
            rate: 400_000.0,
            notes: "C, epoll multi-thread",
        },
        Market {
            name: "OTel Collector 0.92",
            rate: 300_000.0,
            notes: "Go, pipeline model",
        },
        Market {
            name: "Filebeat 8.x",
            rate: 150_000.0,
            notes: "Go, single pipeline",
        },
        Market {
            name: "Splunk UF 9.x",
            rate: 100_000.0,
            notes: "C++, sustained guideline",
        },
        Market {
            name: "Cribl Stream 4.x",
            rate: 80_000.0,
            notes: "Node.js, single worker",
        },
        Market {
            name: "NXLog Enterprise 5.x",
            rate: 50_000.0,
            notes: "C, sync I/O model",
        },
        Market {
            name: "Fluentd 1.16 (Ruby)",
            rate: 50_000.0,
            notes: "Ruby MRI, GIL limited",
        },
        Market {
            name: "Logstash 8.x",
            rate: 30_000.0,
            notes: "JVM, pipeline=1",
        },
        Market {
            name: "CyberboxSIEM Collector",
            rate: cyberbox_recv,
            notes: "Rust, tokio, this run",
        },
    ];
    ...
    let mut sorted = market
        .to_vec()
        .iter()
        .map(|m| (m.name, m.rate, m.notes))
        .collect::<Vec<_>>();
    sorted.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

    for (name, rate, notes) in &sorted {
        let ratio = if cyberbox_recv > 0.0 {
            rate / cyberbox_recv
        } else {
            0.0
        };
        let marker = if *name == "CyberboxSIEM Collector" {
            " ◀ THIS RUN"
        } else {
            ""
        };
