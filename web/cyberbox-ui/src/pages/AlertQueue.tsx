    ...
    <div className="flex flex-col gap-6">
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.9fr)]">
        <Card className="overflow-hidden border-primary/15 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.16),transparent_40%),linear-gradient(145deg,hsl(var(--card)),hsl(var(--card)/0.82))]">
          ...
          <div className="grid gap-3 rounded-[24px] border border-border/70 bg-background/35 p-4">
            <WorkspaceStatusBanner tone={connected ? 'success' : 'warning'} className="rounded-[20px]">
              {connected ? 'Real-time alert stream connected.' : 'Realtime stream unavailable right now.'}
            </WorkspaceStatusBanner>
            {error && <WorkspaceStatusBanner tone="warning" className="rounded-[20px]">{error}</WorkspaceStatusBanner>}
            {statusText && <WorkspaceStatusBanner className="rounded-[20px]">{statusText}</WorkspaceStatusBanner>}
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-[20px] border border-border/70 bg-card/75 p-4"><div className="text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Open</div><div className="mt-2 font-display text-3xl font-semibold text-foreground">{stats.open}</div></div>
              <div className="rounded-[20px] border border-border/70 bg-card/75 p-4"><div className="text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Unassigned</div><div className="mt-2 font-display text-3xl font-semibold text-foreground">{stats.unassigned}</div></div>
            </div>
          </div>
          </CardContent>
        </Card>
      ...
      <section className={cn('grid gap-4', explainId ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : '')}>
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                      <div className="rounded-[18px] border border-border/70 bg-background/35 p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">Source</div><div className="mt-2 text-sm text-foreground">{src}</div></div>
                      <div className="rounded-[18px] border border-border/70 bg-background/35 p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">Destination</div><div className="mt-2 text-sm text-foreground">{dst ?? 'Unavailable'}</div></div>
                      <div className="rounded-[18px] border border-border/70 bg-background/35 p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">Process</div><div className="mt-2 truncate text-sm text-foreground">{proc ?? 'Not provided'}</div></div>
                      <div className="rounded-[18px] border border-border/70 bg-background/35 p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">MITRE</div><div className="mt-2 flex flex-wrap gap-2">{alert.mitre_attack.length ? alert.mitre_attack.slice(0, 2).map((m) => <Badge key={m.technique_id} variant="outline">{m.technique_id}</Badge>) : <span className="text-sm text-muted-foreground">No mapping</span>}</div></div>
                    </div>
                    {open && <div className="mt-4 grid gap-4 rounded-[22px] border border-border/70 bg-background/30 p-4 xl:grid-cols-3"><div className="space-y-3 text-sm"><div><div className="text-muted-foreground">Alert ID</div><div className="break-all font-mono text-xs text-foreground">{alert.alert_id}</div></div><div><div className="text-muted-foreground">Rule ID</div><div className="break-all font-mono text-xs text-foreground">{alert.rule_id}</div></div><div><div className="text-muted-foreground">First seen</div><div className="text-foreground">{new Date(alert.first_seen).toLocaleString()}</div></div></div><div className="space-y-3 text-sm"><div><div className="text-muted-foreground">OS</div><div className="text-foreground">{alert.agent_meta?.os ?? 'Unavailable'}</div></div><div><div className="text-muted-foreground">Group</div><div className="text-foreground">{alert.agent_meta?.group ?? 'Unavailable'}</div></div><div><div className="text-muted-foreground">Assignee</div><div className="inline-flex items-center gap-2 text-foreground"><UserRound className="h-4 w-4 text-muted-foreground" />{alert.assignee ?? 'Unassigned'}</div></div></div><div className="space-y-3">{alert.case_id ? <Button asChild variant="outline" className="w-full justify-between rounded-[18px]"><Link to={`/cases/${alert.case_id}`}>Open linked case<ExternalLink className="h-4 w-4" /></Link></Button> : <div className="rounded-[18px] border border-dashed border-border/80 px-4 py-3 text-sm text-muted-foreground">No case linked yet.</div>}{alert.mitre_attack.map((m) => <div key={`${m.technique_id}-${m.tactic}`} className="rounded-[18px] border border-border/70 bg-card/70 p-3 text-sm"><div className="font-medium text-foreground">{m.technique_id}</div><div className="text-muted-foreground">{m.tactic} · {m.technique_name}</div></div>)}</div></div>}
                  </div>
                </CardContent>
              </Card>
            );
        ...
        {explainId && <Card className="h-fit xl:sticky xl:top-28"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>AI explanation</CardTitle><CardDescription>{activeExplainAlert ? titleFor(activeExplainAlert, titles) : 'Alert context'}</CardDescription></div><Button type="button" variant="ghost" size="icon" onClick={() => setExplainId(null)}><XCircle className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-4">{explainLoading && <div className="rounded-[20px] border border-border/70 bg-background/35 p-4 text-sm text-muted-foreground">Generating analyst summary...</div>}{explainError && <WorkspaceStatusBanner tone="danger" className="rounded-[20px]">{explainError}</WorkspaceStatusBanner>}{!explainLoading && !explainError && explainResult && <><div className="rounded-[20px] border border-border/70 bg-background/35 p-4"><div className="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-primary"><Sparkles className="h-4 w-4" />Summary</div><p className="text-sm leading-6 text-foreground">{explainResult.summary}</p></div><div className="rounded-[20px] border border-border/70 bg-background/35 p-4"><div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Why suspicious</div><p className="text-sm leading-6 text-foreground">{explainResult.why_suspicious}</p></div><div className="rounded-[20px] border border-border/70 bg-background/35 p-4"><div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Likely cause</div><p className="text-sm leading-6 text-foreground">{explainResult.likely_cause}</p></div><div className="rounded-[20px] border border-border/70 bg-background/35 p-4"><div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">False positive likelihood</div><Badge variant="outline">{explainResult.false_positive_likelihood}</Badge></div><div className="rounded-[20px] border border-border/70 bg-background/35 p-4"><div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.28em] text-muted-foreground">Recommended actions</div><div className="space-y-3">{explainResult.recommended_actions.map((action) => <div key={action} className="flex items-start gap-3 text-sm text-foreground"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>{action}</span></div>)}</div></div></>}{!explainLoading && !explainError && !explainResult && <WorkspaceEmptyState title="No explanation ready" body="Choose an alert and run AI Explain to open an analyst summary here." className="min-h-[180px]" />}</CardContent></Card>}
      </section>
    </div>
  );
}
